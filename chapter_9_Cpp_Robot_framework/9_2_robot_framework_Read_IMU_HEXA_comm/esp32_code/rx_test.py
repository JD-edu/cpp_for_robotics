    ...
    try:
        ...
        while True:
            ...
            if ord(byte) == HEADER_BYTE:
                # 헤더 다음의 12바이트를 읽으려고 시도 (총 13바이트 패킷 중 헤더 제외)
                remaining_data = ser.read(12)
                
                # 읽어온 데이터의 길이가 정확히 12바이트인지 확인
                if len(remaining_data) == 12:
                    try:
                        # ">hhhhhh"는 6개의 short (2바이트) 값을 나타내므로 총 12바이트
                        accelX, accelY, accelZ, gX, gY, gZ = struct.unpack(">hhhhhh", remaining_data)
                        print(f"Accel: X={accelX}, Y={accelY}, Z={accelZ}")
                        
                    except struct.error as se:
                        print(f"Struct unpack error: {se}. Data length: {len(remaining_data)}. Data: {remaining_data.hex()}")
